function [DKI, Flair, VOI_K, VOI_F, PixelDimensions] = Read_Images_VOIs_Only_Boundary(model, name)
...
% convert boundary into mask (only appliable to 55,45 Flair cases so far)
VOI_F_Converted = zeros(size(VOI_F));
for k = 1:1:size(VOI_F,3)
    I = VOI_F(:,:,k);
    I = (I == 1);
    VOI_F_Converted(:,:,k) = imfill(I, 'holes');
end
VOI_F = VOI_F_Converted;
